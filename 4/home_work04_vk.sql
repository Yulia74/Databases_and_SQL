...
WHERE p.birthday BETWEEN '2011-01-01' AND '2023-01-01'
GROUP BY m.id
...
SELECT
    ...
    u.firstname,
    u.lastname,
    msg.id AS 'id сообщения'
FROM users u
LEFT JOIN messages msg ON u.id =  msg.from_user_id
WHERE msg.id IS NULL;

-- 4. Из всех друзей пользователя Reuben найдите человека, который больше всех написал ему сообщений
...
SELECT
    ...
    u.lastname,
    msg.id AS 'id сообщения',
    COUNT(msg.id) AS "кол-во сообщений"
FROM users u
JOIN messages msg ON u.id = msg.from_user_id WHERE to_user_id = 1
GROUP BY msg.id;